...
- Viết `docker-compose.yml` để định nghĩa ít nhất hai service: API và AI inference.
- Dùng network `team-internal` để giao tiếp nội bộ giữa các container.
- Chạy API bằng non‑root user trong container và giữ nguyên `HEALTHCHECK` như Lab 04.
- Thêm healthcheck cho AI service để Compose biết khi nào container sẵn sàng.
- Tách cấu hình runtime qua `.env.example` (ví dụ `APP_PORT`, `AI_SERVICE_PORT`, `SERVICE_VERSION`, `AUTH_TOKEN`).
...
- Soạn **checklists/readiness-checklist.md** mô tả checklist readiness và tick khi hoàn thành.
...
Thư mục `src/ai_vision_service` chứa AI Vision API (FastAPI) giống Lab 04. Thư mục `src/ai_service` chứa service AI inference (mock), cung cấp endpoint `/detect` trả về kết quả dummy. Nhóm có thể thay bằng mô hình thực tế (YOLOv8, MediaPipe…).
...
uvicorn ai_vision_service.main:app --app-dir src --host 0.0.0.0 --port 8000
```
